...
import org.youi.chart.engine.model.series.LineSeries;
 ...
 */
@Component
public class LineOptionProcessor extends XYOptionProcessor implements IChartOptionProcessor {

    @Override
    public boolean supports(String chartType) {
        return ChartType.Line.name().equals(chartType);
    }
    ...
    protected ISeries buildSeries(Item item) {
        return new LineSeries().setId(item.getId()).setName(item.getText());
    }
}
