<%@ include file="/WEB-INF/pages/common/include.jsp"%>
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>

<youi:page caption="计量单位编辑" autoLoadData="${param.id!=null}"
	dataSrc="/metadataServices/services/measureUnitManager/getMeasureUnit.json?id=${param.id}">

	<youi:convert name="baseUnitConvert" show="unitCaption" code="id" usecache="false"
				  src="/metadataServices/services/measureUnitManager/getMeasureUnits.json?rate=1"/>

	<youi:form id="form_measureUnit" panel="false" submit="NOT" reset="NOT" action="/metadataServices/services/measureUnitManager/saveMeasureUnit.json"
		idKeys="id">
		<youi:fieldLayout columns="1" labelWidths="100">
			<youi:fieldHidden property="id"  caption="i18n.measureUnit.id"/>
			<youi:fieldSelect convert="measurement" property="measurement"   caption="度量衡"/>
			<youi:fieldText notNull="true" property="unitCaption"  caption="单位名称"/>
			<youi:fieldText notNull="true" property="rate"  caption="换算率"/>
			<youi:fieldSelect convert="baseUnitConvert" property="baseUnitId"  caption="基本单位"/>
			<youi:fieldText property="description"  caption="单位描述"/>
		</youi:fieldLayout>
	</youi:form>
	
	<youi:button name="submit" caption="提交" submitProperty="submit" submitValue="1"/>
	
	<youi:func name="init" params="results">
		if(results&&results.record){
			$elem('form_measureUnit',pageId).form('fillRecord',results.record);
		}
	</youi:func>
	
	<youi:func name="form_measureUnit_afterSubmit" params="results">
		$.youi.messageUtils.showMessage('保存计量单位成功.');
		$elem('form_measureUnit',pageId).form('reset');
		$.youi.pageUtils.closeAndRefreshSubpage(pageId,results.record);//关闭并刷新主页面的subpage组件
	</youi:func>
</youi:page>